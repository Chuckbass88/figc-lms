...
export default async function InvitoDocentePage({ params }: { params: Promise<{ token: string }> }) {
  ...
  if (user) {
    // Già autenticato → mostra messaggio contestuale invece di redirectare
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
        <div className="bg-white rounded-2xl shadow-lg p-8 max-w-md w-full text-center">
          ...
          <h1 className="text-xl font-bold text-gray-900 mb-2">Link di invito docente</h1>
          <p className="text-gray-500 text-sm mb-4">
            Sei già autenticato come <strong>{user.email}</strong>.<br />
            Questo link è pensato per essere condiviso con il nuovo docente che deve registrarsi.
          </p>
          <p className="text-xs text-gray-400 mb-5">Copia e invia questo link al docente tramite email o messaggio.</p>
          <a href="/super-admin/utenti"
