...
export default function RegistrazioneDocenteForm({ token }: Props) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    const res = await fetch('/api/invito-docente/registra', {
      method: 'POST',
    ...
    <form onSubmit={submit} className="space-y-4">
      ...
      <div>
        <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide block mb-1">
          Numero di cellulare <span className="text-gray-400 font-normal normal-case">(opzionale)</span>
        </label>
        <input
          type="tel"
          value={cellulare}
      ...
      <div>
        <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide block mb-1">
          Password <span className="text-gray-400 font-normal normal-case">(min. 8 caratteri)</span>
        </label>
      ...
      <div className="space-y-2.5 pt-1">
        ...
        <label className="flex items-start gap-2.5 cursor-pointer">
          ...
          <span className="text-xs text-gray-600">
            ...
            <a href="/termini" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">
              Termini di Utilizzo
            </a>
          </span>
        </label>
      ...
      >
        {loading ? (
          ...
          <><Check size={14} /> Crea il tuo account docente</>
        )}
      </button>
    </form>
  )
