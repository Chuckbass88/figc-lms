...
export default function ImportaExcelDocenteBtn() {
  ...
  async function handleFile(file: File) {
    ...
    fd.append('file', file)
    const res = await fetch('/api/quiz/importa-libreria-docente', { method: 'POST', body: fd })
    const json = await res.json()
    setLoading(false)
    if (!res.ok) { setError(json.error ?? 'Errore durante l\'importazione'); return }
    setResult(json)
    ...
    <div className="flex flex-col items-start gap-2">
      ...
      {result && (
        <p className="flex items-center gap-1.5 text-sm text-green-700 bg-green-50 rounded-lg px-3 py-2">
          <CheckCircle size={14} /> {result.imported} domande importate{result.skipped > 0 && ` · ${result.skipped} saltate`}
        </p>
      )}
      {error && (
