...
type Corso = { id: string; name: string }
...
export default function ArchivioPaginaClient({
  ...
  function FileRow({ f }: { f: ArchiviFile }) {
    ...
    const areaNome = aree.find(a => a.id === f.area_id)?.nome
      ...
      <div className="flex items-center gap-3 px-5 py-3 border-t hover:bg-gray-50 transition group"
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs mt-0.5 flex gap-2 flex-wrap" style={{ color: 'rgba(27,55,104,0.45)' }}>
            ...
            {areaNome && vista !== 'area' && <span>· {areaNome}</span>}
  ...
  const VISTE = [
    ...
    { key: 'area' as Vista, label: 'Per area', icon: <FolderOpen size={13} /> },
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="flex gap-1 bg-white border border-gray-200 rounded-xl p-1">
          {VISTE.map(v => (
            <button key={v.key} onClick={() => { setVista(v.key); setFiltroArea(''); setFiltroCorso('') }}
        ...
        <div className="space-y-4">
          {aree
            ...
            .map(area => {
              ...
              return (
                <div key={area.id} className="rounded-2xl overflow-hidden bg-white"
                  style={{ border: '1px solid rgba(27,55,104,0.1)' }}>
                  <div className="px-5 py-3 flex items-center justify-between"
                    ...
                    <span className="text-xs" style={{ color: 'rgba(27,55,104,0.45)' }}>{areaFiles.length} file</span>
                  </div>
                  {areaFiles.map(f => <FileRow key={f.id} f={f} />)}
                </div>
              )
            })}
          {/* Senza area */}
            ...
            if (!filtroArea && senzaArea.length > 0) return (
              <div className="rounded-2xl overflow-hidden bg-white" style={{ border: '1px solid rgba(27,55,104,0.1)' }}>
                <div className="px-5 py-3" style={{ borderBottom: '1px solid rgba(27,55,104,0.08)' }}>
                  <h3 className="text-sm font-semibold" style={{ color: 'rgba(27,55,104,0.4)' }}>Senza area</h3>
                </div>
                {senzaArea.map(f => <FileRow key={f.id} f={f} />)}
              </div>
            )
            return null
          })()}
            ...
            .map(corso => {
              ...
              return (
                <div key={corso.id} className="rounded-2xl overflow-hidden bg-white"
                  ...
                  <div className="px-5 py-3 flex items-center justify-between"
                    style={{ background: 'rgba(27,55,104,0.03)', borderBottom: '1px solid rgba(27,55,104,0.08)' }}>
                    <h3 className="text-sm font-semibold" style={{ color: '#1B3768' }}>{corso.name}</h3>
                    <span className="text-xs" style={{ color: 'rgba(27,55,104,0.45)' }}>{corsoFiles.length} file</span>
                  </div>
                  {corsoFiles.map(f => <FileRow key={f.id} f={f} />)}
                </div>
              )
            })}
          {filtrati.filter(f => f.corso_origine_id).length === 0 && (
            <div className="rounded-2xl bg-white px-5 py-10 text-center text-sm"
              style={{ border: '1px solid rgba(27,55,104,0.1)', color: 'rgba(27,55,104,0.4)' }}>
